...
future high intensity cyclotrons and FFAs. It tracks multiple particles
...
`BEAM` (see Chapter <<chp.beam-command,Beam Command>>),
...
line `DISTRIBUTION` (see Chapter <<chp.distribution,Distribution>>)
...
latexmath:[x, p_x, y, p_y, z, p_z] in the local
reference frame. Their units are described in
<<sec.distribution.unitsdistattributes,Units>>.
...
[[sec.opalcycl.tune-calculation-mode]]
...
[[sec.opalcycl.multi-particle-tracking-mode]]
...
will describe this in detail in <<sec.opalcycl.MultiBunch>>.

[[sec.opalcycl.variables]]
...
PX::
  Horizontal canonical momentum [<<sec.distribution.unitsdistattributes,Units>>].
Y::
  Longitudinal position latexmath:[y] of a particle in global
  Cartesian coordinates [m].
PY::
  Longitudinal canonical momentum [<<sec.distribution.unitsdistattributes,Units>>].
Z::
  Vertical position latexmath:[z] of a particle in global Cartesian
  coordinates [m].
PZ::
  Vertical canonical momentum [<<sec.distribution.unitsdistattributes,Units>>].
...
generated by a distribution generator (see Chapter <<chp.distribution,Distribution>>),
...
latexmath:[p_{total}] (defined by the beam energy specified in the <<sec.beam-command.beam-energy,Beam Command>>),
latexmath:[p_{r0}] (`PRINIT`), and latexmath:[p_{z0}] (`PZINIT`) of
the reference particle, defined in the `<<sec.elements.cyclotron,CYCLOTRON>>` 
...
With respect to the external magnetic field map two possible situations
can be considered: in the first situation, the measured field map data
is loaded by the tracker. In most cases, only the vertical field,
latexmath:[B_z], can be measured on the median plane (latexmath:[z=0]).
...
.Coefficients for derivatives with 5-point Lagrange formula
[latexmath%number#eq-CoeffDer]
++++
\begin{aligned}
...
All the partial differential coefficients are computed from the median
plane data by interpolation using Lagrange's 5-point
formula.

In the other situation, 3D magnetic field data for the region of interest
is calculated numerically by building a 3D model using commercial software,
such as TOSCA, ANSOFT and ANSYS during the design phase of a new cyclotron.
If the field on the median plane is calculated, the field off the median plane can be
obtained using the same expansion approach as the measured field map as
described above. In this case the calculated field will be more accurate, especially
at large distances from the median plane i.e. a full 3D field map can be calculated.
...
`TYPE` of `<<sec.elements.cyclotron,CYCLOTRON>>` in the input file.

[[sec.opalcycl.carboncycl-type]]
==== CARBONCYCL type

If `TYPE=CARBONCYCL`, the program requires the latexmath:[B_z] data
which is stored in a sequence shown in <<fig_CYCLField>>.

.2D field map on the median plane with primary direction corresponding to the azimuthal direction, secondary direction to the radial direction
[[fig_CYCLField,Figure {counter:fig-cnt}]]
image::./figures/cyclotron/CarbonFieldFormat.png[scaledwidth=6cm,width=30%]
...
electric field from the <<sec.elements.cyclotron,`CYCLOTRON`>>
element. The magnetic field data (latexmath:[B_z]) are stored in the same
format as <<sec.opalcycl.carboncycl-type,`CARBONCYCL`>>. Regarding
the electric field, the 3D RF field-map can be read from H5Hut
...
<<sec.opalcycl.read-3d-rf-field-map,Read 3D RF field-map>>.
...
If `TYPE=RING`, the program requires the data format like PSI format field file 
_ZYKL9Z.NAR_ and _SO3AV.NAR_, which was given by the measurement. We add 
4 parameters at the header of the file, namely, latexmath:[r_{min}] [mm],
latexmath:[\Delta r] [mm], latexmath:[\theta_{min}][latexmath:[^{\circ}]],
...
[[sec.opalcycl.users-own-field-map]]
...
`CYCLOTRON`, please refer to  <<sec.elements.cyclotron,Cyclotron>>.

[[sec.opalcycl.rffieldmap]]
=== RF field

[[sec.opalcycl.read-rf-voltage-profile]]
...
cavities, see  <<sec.elements.cyclotron,Cyclotron>>.
...
The 3D RF field-map can be read from H5Hut type
...
magnetic field evaluations in each time step latexmath:[\tau]. During
...
[[sec.opalcycl.space-charge]]
=== Space Charge

_OPAL-cycl_ uses the same solvers as _OPAL-t_ to calculate the space
charge effects see Chapter <<chp.fieldsolvers,Field Solver>>.
...
_MAD_ language which will be described in detail in the following
chapters.
...
link:examples/tuning.sh[tuning.sh]
...
_OPAL-cycl_ writes out several output files, some specific to the Tracking mode
(see following sections).

[[sec.opalcycl.input_file_name-.stat]]
==== Statistics output

See <<sec.opalt.input_file_name-.stat,_OPAL-t_ Statistics output>>. The only
difference is in the cyclotron coordinate convention. The z (or s) coordinate
refers to the vertical direction and y corresponds to the longitudinal or beam
travel direction. In addition, _OPAL-cycl_ includes some extra data, as is
described in the following table.

.Additional data stored in statistics output file in _OPAL-cycl_.
[cols="<1,<1,<5",options="header",]
[[tab_opal_cycl_stat,{counter:tab-cnt}]]
|=======================================================================
|Name |Units |Meaning

|R0_x |m |X-component of position of particle with ID 0 (only when
run serial)

|R0_y |m |Y-component of position of particle with ID 0 (only when
run serial)

|R0_s |m |S-component of position of particle with ID 0 (only when
run serial)

|P0_x |m |X-component of momentum of particle with ID 0 (only when
run serial)

|P0_y |m |Y-component of momentum of particle with ID 0 (only when
run serial)

|P0_s |m |S-component of momentum of particle with ID 0 (only when
run serial)
...
[[sec.opalcycl.single-particle-tracking-mode-1]]
==== Single Particle Tracking mode
...
be set using the option `SPTDUMPFREQ` of <<sec.control.option,`OPTION` statement>>.

The phase space data per `STEPSPERTURN` (a parameter in the `<<chp.track,TRACK>>`
...
through a full 360latexmath:[^{\circ}] in `STEPSPERTURN` steps.
...
[[sec.opalcycl.tune-calculation-mode-1]]
==== Tune calculation mode

The tunes latexmath:[\nu_r] and latexmath:[\nu_z] of each energy are
stored in a ASCII file named _tuningresult_.

[[sec.opalcycl.multi-particle-tracking-mode-1]]
==== Multi-particle tracking mode
...
using H5root <<bib.h5root>>, <<bib.schietinger>>. The frequency of the data output can
be set using the `PSDUMPFREQ` option of <<sec.control.option,`OPTION` statement>>.
...
the `SPTDUMPFREQ` option of <<sec.control.option,`OPTION` statement>>.
...
<<sec.control.option,`OPTION` statement>>.

Moreover, `DUMPFIELDS` and `DUMPEMFIELDS` routines (see Chapter <<chp.field_output,Field Output>>)
...
https://github.com/OPALX-project/regression-tests/blob/master/RegressionTests/RingCyclotronMatched/RingCyclotronMatched.in[matchedDistribution.in]

All supplementary files can be found in https://github.com/OPALX-project/regression-tests/tree/master/RegressionTests/RingCyclotronMatched[the matched distribution regression test].

[[sec.opalcycl.output-1]]
===== Output

https://github.com/OPALX-project/regression-tests/blob/master/RegressionTests/RingCyclotronMatched/reference/RingCyclotronMatched.out[matchedDistribution.output]

[[sec.opalcycl.bibliography]]
=== References

anchor:bib.gordon1985[[{counter:bib-cnt}\]]
<<bib.gordon1985>> M. M. Gordon and V. Taivassalo,  https://ieeexplore.ieee.org/document/4333942[_Nonlinear Effects of Focusing Bars Used in the Extraction Systems of Superconducting Cyclotrons_], IEEE Trans. Nucl. Sci. 32, 2447 (1985).

anchor:bib.howison2010[[{counter:bib-cnt}\]]
<<bib.howison2010>> M. Howison et al., http://dx.doi.org/10.1109/CLUSTERWKSP.2010.5613098[_H5hut: A High-Performance I/O Library for Particle-based Simulations_], in 2010 IEEE International Conference on Cluster Computing Workshops and Posters, vol. 1, pp. 1–8 (Heraklion, Crete, 2010).

anchor:bib.h5root[[{counter:bib-cnt}\]]
<<bib.h5root>> link:h5root.html[_H5root: a ROOT Based Graphical User Interface for H5hut_]

anchor:bib.schietinger[[{counter:bib-cnt}\]]
<<bib.schietinger>> T. Schietinger, https://accelconf.web.cern.ch/ICAP2009/papers/thpsc049.pdf[_H5PartROOT - A visualization and post-processing tool for accelerator simulations_], in Proceedings of the 10th International Computational Accelerator Physics conference (ICAP09), pp. 343-346 (San Francisco, CA, USA, 2009).
