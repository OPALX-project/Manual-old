...
[appendix]
[[appendix.opalt-fieldmaps]]

== _OPAL-t_ Field Maps
include::stylesheets/Toggle[]

[[sec.fieldmaps.introduction]]
=== Introduction
...
[[sec.fieldmaps.comments-in-field-maps]]
...
[[sec.fieldmaps.field-map-warnings-and-errors]]
...
3.  Enge function <<bib.enge_fieldmaps>> field map. This type of field map uses Enge
functions to describe the fringe fields of a magnet. Currently, this is
only used for `RBEND` and `SBEND` elements see
<<sec.elements.RBend,RBend (_OPAL-t_)>> and <<sec.elements.SBend,SBend (_OPAL-t_)>>.
...
1DMagnetoStatic::
  1D magnetostatic field map. See <<sec.fieldmaps.1DMagnetoStatic>>.
AstraMagnetoStatic::
  ...
  See <<sec.fieldmaps.AstraMagnetostatic>>.
1DDynamic::
  1D dynamic electromagnetic field map. See <<sec.fieldmaps.1DDynamic>>.
AstraDynamic::
  ...
  small changes. See <<sec.fieldmaps.AstraDynamic>>.
1DProfile1::
  This type of field map specifies the Enge functions (see <<bib.enge_fieldmaps>>) for
  the entrance and exit fringe fields of a magnet. Currently this type
  of field map is only used by `RBEND` and `SBEND` elements
  see <<sec.elements.RBend,RBend (_OPAL-t_)>> and <<sec.elements.SBend,SBend (_OPAL-t_)>>. See <<sec.fieldmaps.1DProfile1>>.
2DElectroStatic::
...
.Ordering of points for 2D field maps in T7 files (left XZ orientation, right ZX orientation)
[[fig_2Dordering,Figure {counter:fig-cnt}]]
image::./figures/Fieldmaps/order.png[scaledwidth=18cm,width=60%]

[[sec.fieldmaps.fastattribute]]
...
be used! <<fig_fieldnoiseexample>> shows three pictures of the
longitudinal phase space after three gun simulations using different
types of field maps. In the first picture we
used a 1DDynamic field map see <<sec.fieldmaps.1DDynamic>> resulting in a smooth
...
[[fig_fieldnoiseexample,Figure {counter:fig-cnt}]]
image::./figures/Fieldmaps/fieldnoise_example.png[scaledwidth=18cm,width=80%]
opal-end
...
latexmath:[\max(|B_{\text{on axis}}|) = 1.0\mathrm{T}].
opal-begin
If the `FAST`
...
[[tab_1DMagnetoStatic,Table {counter:tab-cnt}]]
[cols="<,<,<",grid=none,frame=topbot,options=noheader]
|=======================================================================
...
<<tab_1DMagnetoStatic>>. The first three lines form the file header and
...
1DMagnetoStatic see <<sec.fieldmaps.1DMagnetoStatic>> field maps, _OPAL-t_
...
dimension is contained in the first column of the data.
opal-begin
(_OPAL-t_ does
...
[[tab_AstraMagnetoStatic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
|======================================================================
|AstraDynamic |latexmath:[N_{Fourier}] |TRUE \| FALSE (optional)
...
<<tab_AstraMagnetoStatic>>. The first line forms the file header and
...
latexmath:[\max(|E_{on axis}|) = 1\mathrm{MV/m}].
opal-begin
If the `FAST` switch is
...
[[tab_1DDynamic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
...
A `1DDynamic` field map has the general form shown in <<tab_1DDynamic>>.
...
this field is 2997.924MHz.
opal-begin
(_OPAL-t_ does not provide a `FAST` version
of this map type.)
opal-end
.Layout of an `AstraDynamic` field map file.
[[tab_AstraDynamic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
...
<<tab_AstraDynamic>>. The first line forms the file header and tells
...
[[sec.fieldmaps.1DProfile1]]
=== 1DProfile1

A `1DProfile1` field map is used to define Enge functions <<bib.enge_fieldmaps>> that
...
shown in <<fig_rbend_field_profile>>. In the top part of the figure we
...
When selecting the Enge coefficients, care must be taken to ensure that
the polynomial degree is odd and that the coefficient for the highest
degree is positive. This ensures that the value of the function inside
the dipole is 1 and converges to 0 far outside the dipole. If these two
conditions are not met, then the value of the function increases again
from some point onwards. In the Figure <<fig_Enge_poly>> the effect of
a negative coefficient for the highest degree in the original data is shown. To improve the fieldmap it was chosen some point from which on the polynomial was replaced by a polynomial of the same degree but all coefficients except for the highest order and the constant value are zero. The two remaining coefficients were chosen such that the values of the two polynomials and their derivatives are equal at the connection.

.Effect of a negative coefficient for the highest degree in the original data.
[[fig_Enge_poly,Figure {counter:fig-cnt}]]
image::./figures/Fieldmaps/Enge_poly.png[scaledwidth=16cm,width=80%]

Currently, `1DProfile1` field maps are only implemented for `RBEND` and
`SBEND` elements sees <<sec.elements.RBend,RBend (_OPAL-t_)>>, <<sec.elements.SBend,SBend (_OPAL-t_)>> and <<sec.elements.opaltrbendsbendfields,Bend Fields from 1D Field Maps (_OPAL-t_)>>.

.Example of Enge functions describing the entrance and exit fringe fields of a rectangular bend magnet. The top part of the figure shows the relative field strength on the mid-plane. The bottom part of the figure shows an example of a particle trajectory through the magnet. Note that the magnet field is naturally divided into three regions: entrance fringe field, central field, and exit fringe field.
[[fig_rbend_field_profile,Figure {counter:fig-cnt}]]
image::./figures/Fieldmaps/profile-1.png[scaledwidth=16cm,width=80%]

A `1DProfile1` field map has the general form shown in
<<tab_1DProfile1>>. The first three lines form the file header and tell
_OPAL-t_ how the field map data is being presented:
...
Line 2::
  ...
  see <<sec.fieldmaps.1DProfile1Type1>> and <<sec.fieldmaps.1DProfile1Type2>>. The fourth value on
...
This will be explained in <<sec.fieldmaps.1DProfile1Type1>> and
<<sec.fieldmaps.1DProfile1Type2>>.

.Layout of a `1DProfile1` field map file.
[[tab_1DProfile1,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<,<",]
...
.Illustration of a rectangular bend (`RBEND`, see <<sec.elements.RBend,RBend (_OPAL-t_)>> showing the entrance and exit fringe field regions. latexmath:[\Delta_{1}] is the perpendicular distance in front of the entrance edge of the magnet where the magnet fringe fields are non-negligible. latexmath:[\Delta_{2}] is the perpendicular distance behind the entrance edge of the magnet where the entrance Enge function stops being used to calculate the magnet field. The reference trajectory entrance point is indicated by latexmath:[O_{entrance}]. latexmath:[\Delta_{3}] is the perpendicular distance in front of the exit edge of the magnet where the exit Enge function starts being used to calculate the magnet field. (In the region between latexmath:[\Delta_{2}] and latexmath:[\Delta_{3}] the field of the magnet is a constant value.) latexmath:[\Delta_{4}] is the perpendicular distance after the exit edge of the magnet where the magnet fringe fields are non-negligible. The reference trajectory exit point is indicated by latexmath:[O_{exit}]
[[fig_rbendengetype1,Figure {counter:fig-cnt}]]
image::./figures/Fieldmaps/RBendType1.png[scaledwidth=12cm,width=60%]

.Illustration of a sector bend (`SBEND`, see <<sec.elements.SBend,SBend (_OPAL-t_)>>) showing the entrance and exit fringe field regions. latexmath:[\Delta_{1}] is the perpendicular distance in front of the entrance edge of the magnet where the magnet fringe fields are non-negligible. latexmath:[\Delta_{2}] is the perpendicular distance behind the entrance edge of the magnet where the entrance Enge function stops being used to calculate the magnet field. The reference trajectory entrance point is indicated by latexmath:[O_{entrance}]. latexmath:[\Delta_{3}] is the perpendicular distance in front of the exit edge of the magnet where the exit Enge function starts being used to calculate the magnet field. (In the region between latexmath:[\Delta_{2}] and latexmath:[\Delta_{3}] the field of the magnet is a constant value.) latexmath:[\Delta_{4}] is the perpendicular distance after the exit edge of the magnet where the magnet fringe fields are non-negligible. The reference trajectory exit point is indicated by latexmath:[O_{exit}].
[[fig_sbendengetype1,Figure {counter:fig-cnt}]]
image::./figures/Fieldmaps/SBendType1.png[scaledwidth=12cm,width=60%]

[[sec.fieldmaps.1DProfile1Type1]]
...
<<fig_rbendengetype1>> and <<fig_sbendengetype1>> illustrate the fringe field
regions for an `RBEND` and an `SBEND` element. Referring to the general
field map file shown in <<tab_1DProfile1>>, the values on lines 2 and 3
are given by:
...
points see <<fig_rbendengetype1>> and <<fig_sbendengetype1>>.
...
<<tab_1DProfile1>> are defined. Now
...
see <<sec.fieldmaps.fieldorientation>>. _OPAL-t_ normalizes the field so that
latexmath:[\max(|E_{z, \text{ on axis}}|) = 1\mathrm{MV/m}].

.Layout of a `2DElectroStatic` field map file.
[[tab_2DElectroStatic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
...
see <<sec.fieldmaps.fieldorientation>> and can be one of two formats:
...
[[tab_2DMagnetoStatic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
...
<<tab_2DMagnetoStatic>>. The first three lines form the file header and
tell _OPAL-t_ how the field map data is being presented:
...
on the field orientation see <<sec.fieldmaps.fieldorientation>>. _OPAL-t_
normalizes the field so that
latexmath:[\max(|E_{z,\text{ on axis}}|) = 1\mathrm{MV/m}].

.Layout of a `2DDynamic` field map file.
[[tab_2DDynamic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<,<",]
...
A `2DDynamic` field map has the general form shown in <<tab_2DDynamic>>.
The first four lines form the file header and tell _OPAL-t_ how the
field map data is being presented:

Line 1::
  This tells _OPAL-t_ what type of field file it is (`2DDynamic`) and
  the field orientation see <<sec.fieldmaps.fieldorientation>>.
Line 2::
  This gives the extent of the field map and how many grid spacings
  there are in the fastest changing index direction
  see <<sec.fieldmaps.fieldorientation>>.
...
[[tab_3DMagnetoStatic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
...
<<tab_3DMagnetoStatic>>. The first five lines form the file header and
tell _OPAL-t_ how the field map data is being presented:
...
[[tab_3DMagnetoStatic_Extended,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<",]
...
<<tab_3DMagnetoStatic_Extended>>. The first four lines form the file
...
order of these depend on the field orientation
see <<sec.fieldmaps.fieldorientation>> and can currently only be the format
shown in <<tab_3DMagnetoStatic_Extended>>.
...
[[tab_3DDynamic,Table {counter:tab-cnt}]]
[grid=none]
[frame=topbot]
[options=noheader]
[cols="<,<,<,<,<,<",]
...
A `3DDynamic` field map has the general form shown in <<tab_3DDynamic>>.
...
[[sec.fieldmaps.bibliography]]
=== References

anchor:bib.enge_fieldmaps[[{counter:bib-cnt}\]]
<<bib.enge_fieldmaps>> J. E. Spencer and H. A. Enge, https://www.sciencedirect.com/science/article/abs/pii/0029554X67906842[_Split-pole magnetic spectrograph for precision nuclear spectroscopy_], Nucl. Instrum. Methods 49, 181 (1967).

anchor:bib.superfish[[{counter:bib-cnt}\]]
