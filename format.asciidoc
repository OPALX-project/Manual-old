...
The language dialect here is ajar to _MAD9_, for hard core __MAD8__
...
comment delimiters can be nested; this allows to "comment
...
identifier (see <<sec.format.label>>) and gives a name to the stored command.
...
(see <<sec.format.label>>).
* The `attribute-value` gives it a value (see <<sec.format.attribute>>). When
...
+
Each attribute has a fixed attribute type (see <<sec.format.attribute>>). +
...
expression (see <<sec.format.astring>>).

[[sec.format.attribute]]
...
[cols="<30%,<30%",options="noheader",frame="none",grid="none"]
|======
| String               | <<sec.format.astring>>
| Logical              | <<sec.format.alogical>>
| Real expression      | <<sec.format.areal>>
| Deferred expression  | <<sec.format.adefer>>
| Place                | <<sec.format.aplace>>
| Range                | <<sec.format.arange>>
| Constraint           | <<sec.format.aconstraint>>
| Variable Reference   | <<sec.format.areference>>
| Regular expression   | <<sec.format.wildcard>>
| Array                | <<sec.format.anarray>> 
| Array of logical     | <<sec.format.logarray>>
| Array of real        | <<sec.format.realarray>>
| Array of string      | <<sec.format.strarray>>
| Array of token lists | <<sec.format.tokarray>>
|======
...
[cols="<30%,<30%",options="noheader",frame="none",grid="none"]
|======
| Operators                | <<tab_operator>>
| Functions                | <<tab_realfun>>
| Real functions of arrays | <<tab_compfun>>
| Operand                  | <<sec.format.operand>>
| Random generators        | <<sec.format.adefer>>
|======
...
concatenated using the `&` operator (see <<tab_stroperator>>). An operand
in a string can also use the function `STRING` (see <<tab_stringfun>>).
String values can occur in string arrays (see <<sec.format.anarray>>).

.String Operator in __OPAL__
[[tab_stroperator,Table {counter:tab-cnt}]]
...
[[tab_stringfun,Table {counter:tab-cnt}]]
[cols="<1,<3,<1,<1",options="header",]
|=======================================================================
...
logical expression can occur in logical arrays (see <<sec.format.logarray>>).
...
[[tab_logoperator,Table {counter:tab-cnt}]]
[cols="<1,<3,<1,<1",options="header",]
|=======================================================================
...
[[sec.format.operators]]
=== Operators

An expression can be formed using operators (see <<tab_operator>>) and
functions (see <<tab_realfun>>) acting on operands (see <<sec.format.operand>>).

.Real Operators in __OPAL__
[[tab_operator,Table {counter:tab-cnt}]]
[cols="<1,<3,<1,<1",options="header",]
|=======================================================================
...
[[tab_realfun,Table {counter:tab-cnt}]]
[cols="<1,<3,<1,<1",options="header",]
|=======================================================================
...
[[tab_Array_Functions,Table {counter:tab-cnt}]]
[cols="<1,<3,<1,<1",options="header",]
|=======================================================================
...
(see <<tab_constant>). Their names must not be used for
user-defined labels. Additional symbolic constants may be defined to
simplify their repeated use in statements and expressions.

.Predefined Symbolic Constants
[[tab_constant,Table {counter:tab-cnt}]]
[cols="<,^,^,^",options="header",]
|==========================================================
|_OPAL_ name |Mathematical symbol |Value |Unit
|`PI` |latexmath:[\pi] |3.14159265358979323846 |1
|`TWOPI` |latexmath:[2 \pi] |6.28318530717958647692 |1
...
[[sec.format.element-or-command-attributes]]
...
yet available in _OPAL-t_ and _OPAL-cycl_.
...
A `range` in a beam line (see Chapter <<chp.lines,Beam Lines>>) is
...
* When `place1` refers to a `LINE` (see Chapter <<chp.lines,Beam Lines>>),
the range starts at the *beginning* of this line.
* When `place2` refers to a `LINE` (see Chapter <<chp.lines,Beam Lines>>),
...
(see Chapter <<chp.lines,Beam Lines>>) the range contains the whole of the line.
...
[[sec.format.aconstraint]]
=== Constraints

Please note this is not yet available in _OPAL-t_ and _OPAL-cycl_.
...
[[tab_compfun,Table {counter:tab-cnt}]]
[cols="<1,<3,<1,<1",options="header",]
|=======================================================================
|Function |Meaning |result type |argument type
|`TRUNC(X)` |truncate `X` towards zero (discard fractional part) |real
...
permissible values String values see <<sec.format.astring>>.
